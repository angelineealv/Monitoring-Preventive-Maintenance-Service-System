...
class Login extends CI_Controller
{
  function __construct()
  {
    ...
    if ($this->session->userdata('login') == TRUE) {
      if (session_value('levelid') == 1)
        redirect("transaksi");
      if (session_value('levelid') == 2)
        redirect("transaksi");
      if (session_value('levelid') == 3)
        redirect("transaksi/purchase");
      if (session_value('levelid') == 4)
        redirect("transaksi/purchase");
      if (session_value('levelid') == 7)
        redirect("transaksi/purchase");
      if (session_value('levelid') == 5)
        redirect("transaksi");
      if (session_value('levelid') == 9)
        redirect("transaksi/purchase");
      if (session_value('levelid') == 10)
